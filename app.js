...
const BUILD_ID = "build-27-add-swsh-ultimate";
...
// Core options that make the DS bottom screen respond to touch. These names
// and their current values were read off the running emulator through the
// storage diagnostic (it reported desmume_pointer_type=mouse, which is why
// tapping did nothing) — not guessed from documentation.
const TOUCH_VARS = {
  desmume2015: { desmume_pointer_type: "touch" },
...
function renderEmulatorSettings() {
  ...
  EMULATABLE_PLATFORMS.filter((p, idx, arr) => arr.indexOf(p) === idx).forEach(platform => {
    ...
    if (info.coreOptions) {
      const coreLabels = {
        melonds: "melonds — mais preciso, porém o mais pesado",
        desmume: "desmume — equilíbrio entre velocidade e precisão",
        desmume2015: "desmume2015 — o mais leve, melhor opção em celular"
      };
      coreSelectHtml = `
        <div class="emu-field">
  ...
  perfCard.className = "emu-card";
  const isolated = self.crossOriginIsolated === true;
  perfCard.innerHTML = `
    <h3>🚀 Desempenho</h3>
    <p class="emu-sub">O modo rápido roda o emulador em várias threads. Faz bastante diferença no Nintendo DS, que é o mais pesado.</p>
    <div class="emu-field">
      <label style="display:flex;align-items:center;gap:10px;cursor:pointer">
        <input type="checkbox" id="threadsToggle" ${settings.threads ? "checked" : ""} style="width:20px;height:20px;accent-color:var(--ok)">
        <span>Modo rápido (multithread)</span>
      </label>
      <p class="emu-sub" style="margin-top:8px">
        Estado atual: ${settings.threads
          ? (isolated
              ? "✅ ativo e funcionando."
              : "⏳ ligado, mas ainda não ativo — recarregue a página uma vez.")
          : "desligado."}
      </p>
      <p class="emu-sub">Se depois de ligar algum jogo parar de carregar, é só desmarcar aqui — os cabeçalhos extras exigidos por esse modo podem não combinar com a CDN.</p>
    </div>
  `;
  ...
  if (threadsToggle) threadsToggle.addEventListener("change", async e => {
    ...
    saveSettings();
    // The flag lives in the worker's registration URL, so switching it means
    // replacing the registration and reloading for the new headers to apply.
    try {
      const regs = await navigator.serviceWorker.getRegistrations();
      await Promise.all(regs.map(r => r.unregister()));
      await navigator.serviceWorker.register(settings.threads ? "sw.js?coi=1" : "sw.js");
    } catch (err) {
      console.error("Falha ao trocar o service worker:", err);
    }
    toast(settings.threads ? "Modo rápido ligado, recarregando…" : "Modo rápido desligado, recarregando…");
  ...
  const srcdoc = `<!DOCTYPE html><html><head><meta charset="utf-8">
    ...
    <body>
      ...
      <script>
        ...
        // "lockMouse" (an EmulatorJS setting, seen enabled in the diagnostic)
        // grabs the pointer, which stops taps from landing on the DS touch
        // screen; the touch vars below come from the running core's own
        // option list.
        window.EJS_defaultOptions = Object.assign(
          { "save-state-location": "browser", "lockMouse": "disabled" },
        ...
        if (self.crossOriginIsolated) window.EJS_threads = true;
        ...
        // ---- Our own SRAM (in-game save) persistence -------------------
        // EmulatorJS mirrors /data/saves into IndexedDB itself, but that
        // mirror is wiped at the start of the next session (the file we
        // verified on disk was gone on the following load). So we read the
        // .srm straight out of the emulator's virtual filesystem and keep a
        // copy in the app's own IndexedDB, then write it back on startup.
        var SAVE_DIR = "/data/saves";
        var restoreAttempted = false;

        function getFS() {
          try { return window.EJS_emulator && window.EJS_emulator.gameManager && window.EJS_emulator.gameManager.FS; }
          catch (e) { return null; }
        }
        ...
        function collectSaveFiles() {
          var FS = getFS();
          if (!FS) return null;
          var paths = walkFiles(FS, SAVE_DIR, []);
          var files = [];
        ...
        function writeSaveFiles(files) {
          var FS = getFS();
          if (!FS || !files || !files.length) return 0;
          var written = 0;
          files.forEach(function (f) {
            try {
              var dir = f.path.slice(0, f.path.lastIndexOf("/"));
              if (FS.mkdirTree) FS.mkdirTree(dir);
              FS.writeFile(f.path, new Uint8Array(f.data));
              written++;
            } catch (e) { /* path not writable in this build, skip */ }
          });
          return written;
        }
        // Cheap content signature, so the periodic check can tell an actual
        // in-game save apart from "nothing changed since last time" — that
        // avoids rewriting identical bytes every few seconds, and lets the
        // app confirm to the player exactly when their save was captured.
        function signature(files) {
          return files.map(function (f) {
            var sum = 0;
            for (var i = 0; i < f.data.length; i++) sum = (sum + f.data[i] * (i % 31 + 1)) >>> 0;
            return f.path + ":" + f.data.length + ":" + sum;
          }).join("|");
        }
        var lastSignature = null;

        // A per-game settings entry saved by EmulatorJS can override
        // EJS_defaultOptions, so also push the touch options straight into
        ...
        function applyTouchVars() {
          ...
          if (!gm || typeof gm.setVariable !== "function") {
            notifyParent("touch-config", "Este build não expõe setVariable — não deu para forçar o modo de toque.");
            return;
          }
          var applied = [], failed = [];
        ...
        // The core only writes the .srm to its filesystem on its own schedule
        // ("System Save interval", 5 minutes by default), so polling the FS
        // can't see a save until then. Ask the core to flush now — this is
        // the same operation its timer performs. Name taken from the
        // library's documented saveSaveFiles event; guarded because we can't
        // verify it exists in every build.
        function requestCoreFlush() {
          try {
            var gm = window.EJS_emulator && window.EJS_emulator.gameManager;
            if (gm && typeof gm.saveSaveFiles === "function") { gm.saveSaveFiles(); return true; }
          } catch (e) { /* not available in this build */ }
          return false;
        }

        function pushSaveToParent(reason) {
          var files = collectSaveFiles();
          if (!files || !files.length) return false;
          var sig = signature(files);
          var changed = sig !== lastSignature;
          if (!changed && reason === "interval") return false; // nothing new to store
          lastSignature = sig;
          parent.postMessage({ source: "pkm-emu", status: "sram-data", reason: reason, changed: changed, files: files }, "*");
          return true;
        }

        var autoSaveTimer = null;
        function startAutoSave() {
          // Guarded: EJS_onGameStart fires again after the restore restart,
          // and we don't want a second interval stacking on the first.
          if (autoSaveTimer) return;
          // Take the current state as the baseline, so the save we just
          // restored isn't reported back as a brand-new change.
          var initial = collectSaveFiles();
          if (initial && initial.length) lastSignature = signature(initial);
          // 1s poll: it's just a checksum over a few KB of SRAM, so the cost
          // is negligible and the "save captured" confirmation feels instant.
          // Flushing is asked for slightly less often (it actually writes),
          // and the read happens right after so it sees the fresh bytes.
          var tick = 0;
          autoSaveTimer = setInterval(function () {
            if (tick++ % 2 === 0) requestCoreFlush();
            pushSaveToParent("interval");
          }, 1000);
        }
        ...
        window.EJS_onGameStart = function () {
          ...
          if (!restoreAttempted) {
            ...
            // Ask the parent for a previously stored save for this game.
            // Autosave only starts once that round-trip finishes, so a blank
            // freshly-booted .srm can never overwrite a good stored save.
            setTimeout(function () { notifyParent("sram-request"); }, 400);
          } else {
            startAutoSave();
          }
        };
          ...
          if (e.data.type === "pkm-sram-restore") {
            ...
            if (incoming.length && hasSaveFilesAlready(incoming)) {
              ...
              try {
                ...
                notifyParent("sram-error", "Save restaurado, mas não consegui reiniciar o núcleo: " + err.message);
              }
            }
            startAutoSave();
          } else if (e.data.type === "pkm-sram-collect") {
            if (!pushSaveToParent("final")) {
              parent.postMessage({ source: "pkm-emu", status: "sram-data", reason: "final", files: [] }, "*");
            }
          }
        });

        window.addEventListener("pagehide", function () { pushSaveToParent("pagehide"); });

        // Replace EmulatorJS's built-in on-screen notification (the blocky
        // red "SAVE STATE ARMAZENADO..." text) with our own nicer-looking
        // overlay. displayMessage(text, duration) is a real documented API
        // on the EJS_emulator instance, used internally for exactly these
        // messages — so patch it once it exists instead of fighting its CSS.
        function patchDisplayMessage() {
          if (!window.EJS_emulator || window.EJS_emulator.__pkmPatched) return;
          window.EJS_emulator.__pkmPatched = true;
          window.EJS_emulator.displayMessage = function (text, duration) {
            var el = document.getElementById("pkmMessage");
            if (!el) {
              el = document.createElement("div");
              el.id = "pkmMessage";
              el.style.cssText = "position:absolute;top:14px;left:50%;transform:translateX(-50%);" +
                "background:rgba(10,14,26,.92);color:#f7f8fc;padding:9px 16px;border-radius:12px;" +
                "font:600 13px/1.4 system-ui,-apple-system,sans-serif;z-index:9999;max-width:88%;" +
                "text-align:center;box-shadow:0 8px 20px rgba(0,0,0,.4);pointer-events:none;";
              document.body.appendChild(el);
            }
            el.textContent = text;
            el.style.display = "block";
            clearTimeout(el._pkmTimer);
            el._pkmTimer = setTimeout(function () { el.style.display = "none"; }, duration || 3000);
          };
        }
        var patchInterval = setInterval(function () {
          patchDisplayMessage();
          if (window.EJS_emulator) clearInterval(patchInterval);
        }, 300);

        // Ground-truth diagnostic: list IndexedDB databases actually present
        // in this frame's origin, and the keys stored in /data/saves (the
        // library's own save-file storage), on demand — instead of trusting
        // callback names/behavior that might not even exist in this build.
        function listStoreKeys(dbName) {
          return new Promise(function (resolve) {
            var req = indexedDB.open(dbName);
            req.onsuccess = function () {
              var db = req.result;
              var storeNames = Array.from(db.objectStoreNames);
              if (!storeNames.length) { resolve(dbName + ": (sem object stores)"); return; }
              var tx = db.transaction(storeNames, "readonly");
              var parts = [];
              var remaining = storeNames.length;
              storeNames.forEach(function (storeName) {
                var keysReq = tx.objectStore(storeName).getAllKeys();
                keysReq.onsuccess = function () {
                  parts.push(storeName + ": [" + keysReq.result.join(", ") + "]");
                  if (--remaining === 0) resolve(dbName + " -> " + parts.join(" ; "));
                };
            ...
            req.onerror = function () { resolve(dbName + ": (erro ao abrir)"); };
          });
        }
        window.addEventListener("message", function (e) {
          if (!e.data || e.data.type !== "pkm-check-storage") return;
          if (!indexedDB.databases) {
            notifyParent("storage-info", "indexedDB.databases() não é suportado neste navegador.");
            return;
          }
          indexedDB.databases().then(function (dbs) {
            var names = dbs.map(function (d) { return d.name; });
            var list = dbs.map(function (d) { return d.name + " (v" + d.version + ")"; }).join(", ") || "nenhum banco encontrado";
            // Check both the regular save-file store AND the save-state
            // store, so we can tell whether "Save State" survives a
            // close+reopen even if the automatic SRAM save doesn't.
            var toCheck = names.filter(function (n) { return n.indexOf("saves") !== -1 || n.toLowerCase().indexOf("state") !== -1; });
            Promise.all(toCheck.map(listStoreKeys)).then(function (results) {
              var keysInfo = results.length ? results.join(" || ") : "(nenhum banco de saves/states encontrado)";
              // Also dump how EmulatorJS persists its own settings (the
              // "System Save interval" the user changed by hand lives in
              // localStorage), plus which gameManager methods this build
              // actually exposes — both are needed to apply that interval
              // automatically for every game instead of per game by hand.
              var ls = [];
              try {
                for (var k = 0; k < localStorage.length; k++) {
                  var key = localStorage.key(k);
                  var val = localStorage.getItem(key) || "";
                  ls.push(key + "=" + (val.length > 120 ? val.slice(0, 120) + "…" : val));
                }
              } catch (e) { ls.push("(localStorage inacessível: " + e.message + ")"); }
              var methods = [];
              try {
                var gm = window.EJS_emulator && window.EJS_emulator.gameManager;
                if (gm) {
                  for (var p in gm) { if (typeof gm[p] === "function") methods.push(p); }
                  var proto = Object.getPrototypeOf(gm);
                  if (proto) Object.getOwnPropertyNames(proto).forEach(function (p) {
                    if (methods.indexOf(p) === -1 && typeof gm[p] === "function") methods.push(p);
                  });
                }
              } catch (e) { methods.push("(erro: " + e.message + ")"); }
              // Dump the emulator's own option/settings objects. The DS
              // touchscreen is driven by a libretro core option whose exact
              // name varies per core, so read the real ones off the running
              // instance instead of guessing names.
              var opts = [];
              try {
                var em = window.EJS_emulator;
                if (em) {
                  Object.keys(em).forEach(function (p) {
                    if (!/option|setting|config|control/i.test(p)) return;
                    var v = em[p];
                    if (v && typeof v === "object") {
                      var allKeys = Object.keys(v);
                      // Only surface the ones plausibly related to the DS
                      // touchscreen — the full list is far too long to read.
                      var relevant = allKeys.filter(function (k2) { return /touch|pointer|mouse|screen|layout|cursor/i.test(k2); });
                      var inner = relevant.slice(0, 20).map(function (k2) {
                        var val = v[k2];
                        if (val && typeof val === "object") {
                          // core options usually look like {options:[...], default:...}
                          var vals = val.options || val.values || null;
                          return k2 + "{" + (vals ? vals.join("/") : Object.keys(val).join(",")) + "}";
                        }
                        return k2 + "=" + val;
                      });
                      opts.push(p + " [" + allKeys.length + " no total] relevantes: " + (inner.join(" ; ") || "nenhuma"));
                    }
                  });
            ...
            notifyParent("storage-info", "Erro ao listar: " + err.message);
          });
        });
...
// Index of the game currently open in the player, so incoming SRAM data
// from the iframe can be filed under the right game.
let playingIndex = null;
let finalSaveResolve = null;
...
window.addEventListener("message", async (e) => {
  ...
  } else if (e.data.status === "storage-info") {
    statusEl.hidden = false;
    statusEl.textContent = "🗄️ " + e.data.message;
    // The dump can be long; keep it scrollable instead of pushing the game
    // off-screen, and mirror it to the console for easier copying.
    statusEl.style.maxHeight = "40vh";
    statusEl.style.overflow = "auto";
    console.log("[diagnóstico]", e.data.message);
  } else if (e.data.status === "sram-request") {
    // The emulator just booted and wants any save we kept for this game.
    // Always reply — even with nothing — so the iframe knows the restore
    // round-trip is done and can safely start its autosave loop.
    let stored = null;
    if (playingIndex !== null) {
      try { stored = await idbGet("gameSaves", playingIndex); } catch (err) { console.error("Falha ao ler save guardado:", err); }
    }
    const iframe = $("#emuFrame");
    iframe?.contentWindow?.postMessage({ type: "pkm-sram-restore", files: stored || [] }, "*");
  } else if (e.data.status === "sram-data") {
    if (playingIndex !== null && e.data.files && e.data.files.length) {
      await idbPut("gameSaves", playingIndex, e.data.files);
      // Only speak up when the save actually changed (i.e. right after the
      // player saved in-game) or when closing — not on every silent check.
      if (e.data.changed) toast("💾 Progresso do jogo salvo.");
      else if (e.data.reason === "final") toast("💾 Progresso do jogo salvo.");
    }
    if (e.data.reason === "final" && finalSaveResolve) { finalSaveResolve(); finalSaveResolve = null; }
  } else if (e.data.status === "touch-config") {
    console.log("[toque]", e.data.message);
    if (/falhou|não expõe/.test(e.data.message)) {
      statusEl.hidden = false;
      statusEl.textContent = "⚠️ " + e.data.message;
    }
  } else if (e.data.status === "sram-restored") {
    toast("💾 " + e.data.message);
  } else if (e.data.status === "sram-error") {
    statusEl.hidden = false;
    statusEl.textContent = "⚠️ " + e.data.message;
  }
});

function checkEmuStorage() {
  const iframe = $("#emuFrame");
  if (!iframe || !iframe.contentWindow) { toast("Emulador não está carregado."); return; }
  iframe.contentWindow.postMessage({ type: "pkm-check-storage" }, "*");
}
...
async function closePlayer() {
  // Grab the in-game save one last time before navigating away. This is a
  // quick FS read + IndexedDB write (not the old 5s wait), and it's capped
  // so a non-responding emulator can never block closing.
  const iframe = $("#emuFrame");
  if (iframe && iframe.contentWindow && playingIndex !== null) {
    const done = new Promise(resolve => { finalSaveResolve = resolve; });
    iframe.contentWindow.postMessage({ type: "pkm-sram-collect" }, "*");
    await Promise.race([done, new Promise(r => setTimeout(r, 1500))]);
    finalSaveResolve = null;
  }
...
async function init() {
  ...
  on("#checkStorageBtn", "click", checkEmuStorage);
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => {
    // The ?coi=1 flag tells the worker to add the COOP/COEP headers that
    // SharedArrayBuffer (and therefore multithreaded cores) require.
    navigator.serviceWorker.register(settings.threads ? "sw.js?coi=1" : "sw.js").catch(() => {});
  });
